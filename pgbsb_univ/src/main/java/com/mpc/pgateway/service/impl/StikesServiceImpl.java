...
import com.mpc.pgateway.router.UNIV_CODE;
...
@Service(value="stikesService")
public class StikesServiceImpl extends ComposeDataUMPP implements UMPService {
	private Logger log = Logger.getLogger(getClass());
	@Autowired
	RestResource stikesRestResource;
	...
	@Value("${kode.bank.stikes}")
	private String kodeBank;
	@Value("${password.bank.stikes}")
	...
	public ISOMsg Inquiry(ISOMsg isoMsg) throws ISOException {
		...
		try {
			resp = stikesRestResource.getWebResource()
					.path("inquiry")
		...
		if(resp != null) {
			...
			if(Integer.parseInt(resp.getCode()) == 0) {
				isoMsg.set(48, composeDataResponseInqToIST(resp,String.valueOf(UNIV_CODE.STIKES.code)));
